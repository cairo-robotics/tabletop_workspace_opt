...
class ImageTools:
    ...
    def perform_yolo_inference(self, frame, confidence_threshold=0.6):
        """
        Run YOLO inference on a single frame using this instance's YOLO model.

        This is the same logic as `FusionVision/utils.py::perform_yolo_inference`, but
        baked into `ImageTools` so callers don't need to import it.
        """
    ...
    def process_and_extract_objects(self, color_image, depth_image, intrinsics: o3d.camera.PinholeCameraIntrinsic, depth_scale=1.0, result_image: bool = True):
        ...
        """
        # --- 1. YOLO Detection ---
        all_detections, _ = self.perform_yolo_inference(
            color_image, confidence_threshold=self.yolo_conf_threshold
        )
