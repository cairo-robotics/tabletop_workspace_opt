...
def base_board_constancy(observations, T_hand_cam):
    Ts = []
    for obs in observations:
        T_base_hand = obs["T_base_hand"]
        T_board_cam = obs["T_tag_cam"]
        T_cam_board = invT(T_board_cam)
        T_base_board = T_base_hand @ T_hand_cam @ T_cam_board
        Ts.append(T_base_board)
...
class AprilTagCalibrationNode:
    def __init__(self):
        ...
        self.wait_for_button = rospy.get_param('~wait_for_button', False)  # Wait for button press before capturing
    ...
    def camera_info_callback(self, msg):
        ...
        if self.cam_matrix is None:
            ...
            rospy.loginfo(f"[cam_info] frame_id={msg.header.frame_id} fx={self.cam_matrix[0,0]:.2f} fy={self.cam_matrix[1,1]:.2f}")
    ...
    def capture_observation_callback(self, req):
        ...
        msg = f"Observation {len(self.calib.observations)} captured"
        rospy.loginfo(msg)
    ...
    def solve_translation_only(self, R_hc):
        """
        observations: list of dicts with keys:
        - "T_base_hand": 4x4
        - "T_tag_cam":   4x4 (tag -> cam)
        R_hc: 3x3 known rotation from hand -> cam
        Returns:
        t_hc: (3,) translation hand->cam
        t_bt: (3,) translation base->tag (nuisance const)
        residuals, rank, s: from np.linalg.lstsq
        """
        A_blocks = []
        b_blocks = []
        ...
        for obs in self.calib.observations:
            T_bh = obs["T_base_hand"]
            T_tc = obs["T_tag_cam"]  # tag -> cam

            translations.append(T_tc[:3, 3])
            ...
            # Build one 3x6 block: [R_bh, -I] [t_hc; t_bt] = rhs
            A_i = np.hstack([R_bh, -I])

            rhs_i = -t_bh - (R_bh @ R_hc @ t_ct)

            A_blocks.append(A_i)
            b_blocks.append(rhs_i.reshape(3, 1))

        print("mean/std of T_tag_cam translations: ", np.mean(translations, axis=0), np.std(translations, axis=0))
        A = np.vstack(A_blocks)           # (3N, 6)
        b = np.vstack(b_blocks).reshape(-1)  # (3N,)

        x, residuals, rank, s = np.linalg.lstsq(A, b, rcond=None)
        t_hc = x[:3]
        t_bt = x[3:]
        return t_hc, t_bt, residuals, rank, s
    ...
    def move_to_pose(self, target_pos, target_quat):
        ...
        while (rospy.Time.now() - start_time).to_sec() < move_duration:
            ...
            if pos_error_norm < self.position_tolerance:
                rospy.loginfo(f"✓ Reached target pose (pos: {pos_error_norm*1000:.1f}mm)")
                break
    ...
    def run_auto_calibration(self):
        ...
        for i, pose in enumerate(poses):
            ...
            t = T_board_cam[:3,3]
            rospy.loginfo(f"[board_cam] t = {t} (expect z>0, ~0.5m)")
        ...
        pitch_deg = 0
        yaw_deg = 0
        R_hc = Rotation.from_euler("xyz", [roll_deg, pitch_deg, yaw_deg], degrees=True).as_matrix()
        t_hc, t_bt, residuals, rank, s = self.solve_translation_only(R_hc)
        T_hand_cam = np.eye(4)
        T_hand_cam[:3, :3] = R_hc
        T_hand_cam[:3, 3]  = t_hc

        rospy.loginfo("=== Translation-only result (hand->cam) ===")
        rospy.loginfo(f"t_hc (m): {t_hc}")
        rospy.loginfo(f"(nuisance) t_bt (base->tag) (m): {t_bt}")
        rospy.loginfo(f"residuals: {residuals}, rank: {rank}")

        # This is what set_cams_transforms expects:
        rpy = Rotation.from_matrix(R_hc).as_euler("xyz", degrees=True)
        rospy.loginfo(
            f"python3 set_cams_transforms.py {self.ee_frame} {self.camera_frame} "
            f"{t_hc[0]} {t_hc[1]} {t_hc[2]} {rpy[0]} {rpy[1]} {rpy[2]}"
        )
